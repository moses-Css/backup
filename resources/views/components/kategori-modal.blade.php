<div x-data="{ open: false }" x-show="open"  @open-modal.window="if ($event.detail === 'kategori-modal') open = true" @close-modal.window="if ($event.detail === 'kategori-modal') isOpen = false" class="fixed inset-0 flex items-center justify-center z-50" style="display: none;">
    ...
    <div x-show="open"
        ...
        <form method="POST" action="{{ route('kategoris.store') }}">
            ...
            <x-input-label for="nama" :value="__('Nama Kategori')" class="mb-2" />
            <input type="text" name="nama" placeholder="Contoh : Sosial" class=" w-full p-4 rounded-full border border-neutralGray2" required>
